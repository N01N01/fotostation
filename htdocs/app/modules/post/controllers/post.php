...
//                break;
    ...
    public function ajax_delete_page(){
        $this->db->delete(FACEBOOK_GROUPS, "id  = '".post("id")."'".getDatabyUser());
        unset_session("name");
        ms(array(
            "st"    => "success",
            "label" => "bg-light-green",
...
//            if (!empty($ids)) {
//                foreach ($ids as $id) {
//                    $POST = $this->model->get('*', FACEBOOK_GROUPS, "id = '{$id}'" . getDatabyUser());
//                    if (!empty($POST)) {
//                        switch (post("action")) {
//                            case 'delete':
//                                $this->db->delete(FACEBOOK_GROUPS, "account_id = '" .  $data["name"] . "'");
